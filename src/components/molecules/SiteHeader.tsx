  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-[200] flex items-center justify-between px-6 transition-all duration-300 h-16 ${viewMode === "detailed" ? "bg-black/60 backdrop-blur-xl border-b border-white/10" : "bg-transparent pointer-events-none"}`}>
      <div className={`logo appear appear--scale drop-shadow-[0_4px_12px_rgba(0,0,0,0.8)] ${viewMode === "simple" ? "pointer-events-auto" : ""}`} aria-label="Speechifier" style={{ "--d": "0.08s" } as React.CSSProperties}>
        ...
        <span className={`${instrument.className} text-[1.2em] font-normal tracking-wide`}>Speechifier</span>
      </div>
        ...
        {hasFile && onToggleView ? (
          <div className="flex items-center bg-white/10 rounded-lg p-1 border border-white/10">
            <button
              ...
              className={`px-3 py-1.5 text-[13px] font-medium rounded-md transition-all duration-300 ${viewMode === "simple" ? "bg-white/20 text-white shadow-sm cursor-default" : "text-white/70 hover:text-white cursor-pointer"}`}
            >
              Simple View
            </button>
            <button
              onClick={() => onToggleView("detailed")}
              className={`px-3 py-1.5 text-[13px] font-medium rounded-md transition-all duration-300 ${viewMode === "detailed" ? "bg-white/20 text-white shadow-sm cursor-default" : "text-white/70 hover:text-white cursor-pointer"}`}
            >
              Detailed View
            </button>
          </div>
